...
import sys
class HTTPServer:
    #root = "/home/lenovo/IOS/tws-m2/www"
    ...
    def get_data(self, uri):
        ...
        # print(sample[1])
        if(sample[1]=="bin"):
            loc="/home/day-1Assignments/day-10/"+sample[2]
            # print(loc)
            ...
            if pid:
                ...
                os.execv(loc,sys.argv)
                sys.stdout.flush()
            return 200, "text/html", len(data), data.encode()
        if(uri=="/"):
            data=""
            data += "<h2>Directory listing "+"</h2>"
            listdirec = os.listdir(loc)
            for i in listdirec:
                data+="<li><a href="+uri+i+">"+i+"</a></li>"
            return 200, "text/html", len(data), data.encode()
        loc += uri;
        #print(loc)
        if(os.path.isdir(loc)):
            ...
            type = mimetypes.MimeTypes().guess_type(loc)[0]
